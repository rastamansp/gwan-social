...
function socialScoreForUser(userId: string, domain: FixtureDomain, fallback = 4.0): number {
...
export function getProfileRatedEntriesForReviewee(
  domain: FixtureDomain,
  revieweeId: string,
): ProfileRatedEntry[] {
  const postsById = new Map(domain.posts.map((p) => [p.id, p]))
  return profileRatedEntriesFromDomain(domain, revieweeId, postsById)
}
...
export function getFriendUserIdsFor(domain: FixtureDomain, userId: string): string[] {
  return sessionFriendIds(domain, userId)
}
